...
export default class Demina extends EventEmitter {
  ...
  private async setUp(client: Client, options?: Options) {
    ...
    if (mongoUri) {
      ...
      if (showWarns) {
        console.warn(
          'Demina > No MongoDB connection URI provided. Some features might not work!'
        )
      }
    ...
    if (
      ...
      throw new Error(
        "Demina > The 'commands' directory must be an absolute path. This can be done by using the 'path' module."
      )
    }
    ...
    if (
      ...
      throw new Error(
        "Demina > The 'features' directory must be an absolute path. This can be done by using the 'path' module."
      )
    }
  ...
  public setMongoPath(mongoPath: string | undefined): Demina {
    console.warn(
      'Demina > .setMongoPath() no longer works as expected. Please pass in your mongo URI as a "mongoUri" property using the options object.'
    )
    return this
  }
  ...
  public setDisplayName(displayName: string): Demina {
  ...
  public setDefaultPrefix(defaultPrefix: string): Demina {
  ...
  public setPrefix(guild: Guild | null, prefix: string): Demina {
    if (guild) {
  ...
  public setColor(color: ColorResolvable | null): Demina {
    this._color = color
    return this
  }
  ...
  public setCategorySettings(category: ICategorySetting[]): Demina {
    for (let { emoji, name, hidden, customEmoji } of category) {
      ...
      if (this.isEmojiUsed(targetEmoji)) {
        console.warn(
          `Demina > The emoji "${targetEmoji}" for category "${name}" is already used.`
        )
      }
  ...
  public setTagPeople(tagPeople: boolean): Demina {
  ...
  public setBotOwner(botOwner: string | string[]): Demina {
  ...
  public setDefaultLanguage(defaultLanguage: string): Demina {
