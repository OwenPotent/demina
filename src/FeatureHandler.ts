...
class FeatureHandler {
  ...
  constructor(
    ...
    console.log(
      `Demina > Loaded ${amount} listener${amount === 1 ? '' : 's'}.`
    )
    ;(async () => {
  ...
  private registerFeature = (file: any, fileName: string) => {
    ...
    if (config) {
      ...
      if (missing.length && this._instance.showWarns) {
        console.warn(
          `Demina > Feature "${fileName}" has a config file that doesn't contain the following properties: ${missing}`
        )
      }
    } else if (this._instance.showWarns) {
      console.warn(
        `Demina > Feature "${fileName}" does not export a config object.`
      )
    }
    ...
    if (config && config.loadDBFirst === true) {
      console.warn(
        `Demina > config.loadDBFirst in features is no longer required. MongoDB is now connected to before any features or commands are loaded.`
      )
      // waitingForDB.push({
      //   func,
      //   client: this._client,
